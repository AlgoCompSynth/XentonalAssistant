...
#' @title Synthesize a Single Note
#' @name note_synth
#' @description Generates a Wave object for a single frequency
#' @export note_synth
#' @param freq The frequency of the note in Hz
#' @param signal Signal type: "sine", "tria", "square" or "saw"
#' @param duration_sec Duration in seconds, default 1
#' @param velocity MIDI velocity (0-127), default 100
#' @param sample_rate_hz Sample rate, default 48000
#' @param bit_width Bit width (8, 16, 24, 32, 64), default 24
#' @returns A tuneR Wave object
note_synth <- function(
  freq,
  signal = "tria",
  duration_sec = 1,
  ...
  chord_synth(freq, signal = signal, duration_sec = duration_sec, velocity = velocity, sample_rate_hz = sample_rate_hz, bit_width = bit_width)
}
